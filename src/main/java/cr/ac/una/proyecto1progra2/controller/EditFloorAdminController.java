...
public class EditFloorAdminController extends Controller implements Initializable {
    ...
    @FXML
    private Label LabelCanEscritorios;
    @FXML
    private Label LabelCanSalasComunes;
    @FXML
    private Label LabelCantAreasComunes;
    @FXML
    private Label LabelCantEspaciosLibres;
    @FXML
    private ComboBox<String> ComboBoxPiso;
    @FXML
    private GridPane gridMatrix;
    @FXML
    private Label LabelCapacidadTotal;
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        scrollBar.setMin(0);
        scrollBar.setMax(1); // se actualiza dinámicamente luego

        // Cuando el ScrollBar se mueve, se cambia el translateY del contenido
        scrollBar.valueProperty().addListener((obs, oldVal, newVal) -> {
            double totalHeight = hbox.getHeight();
            double viewHeight = ((Region) hbox.getParent()).getHeight();
            double maxScroll = totalHeight - viewHeight;
            hbox.setTranslateY(-newVal.doubleValue() * maxScroll);
        });

        // Para actualizar el maxScroll si el tamaño cambia
        hbox.heightProperty().addListener((obs, oldVal, newVal) -> {
            double viewHeight = ((Region) hbox.getParent()).getHeight();
            double maxScroll = newVal.doubleValue() - viewHeight;
    ...
    public StackPane crearCeldaEspacio(SpaceVisual espacio) {
        ...
        if (nombre.contains("sala")) {
            ...
            rect.setFill(Color.web("#9C27B0"));
        } else if (nombre.contains("libre")) {
            rect.setFill(Color.web("#4CAF50")); // gris oscuro
        } else if (nombre.contains("e")) {
            rect.setFill(Color.web("#FF5722"));
        } else {
        ...
        for (int row = 0; row <= filas - spanRow; row++) {
            for (int col = 0; col <= columnas - spanCol; col++) {
                ...
                for (SpaceVisual ocupadoEspacio : espaciosAgregados) {
                    if (ocupadoEspacio == espacioSeleccionado) {
                        continue;
                    }
